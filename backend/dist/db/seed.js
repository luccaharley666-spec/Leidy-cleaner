...
async function seedDatabase() {
    try {
        ...
        if (!process.env.SKIP_ADMIN_SEED) {
            const existingAdmin = await query("SELECT COUNT(*) as count FROM users WHERE role = 'admin'");
        ...
        if (existingServices[0].count === 0) {
            ...
            for (const service of services) {
                await query(`INSERT INTO services (name, description, category, base_price, duration_minutes, is_active)
